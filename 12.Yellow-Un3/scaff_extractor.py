...
scaffold = "HG992258.1"                                            #Scaffold where the gene is present, narrows the search space
Species = "Nymphalis_polychloros"                                      #Species to search                  
exon_numbers = "2345678"
scaff_area, start_area, end_area = 0,11980037, 11993326             #if a small exon, and the area for the exon is known, use this to first search in the small area to make blast results easier first parameter - initialize the subsearch, start and end for the coordinates start and end
done_species_temp_switch, temp_species =0, ["Laspeyria_flexula"] 
...
#    print
    ...
    gene = "Yellow-Un1"                                                 #Gene to search    
    gene_folder = "10.Yellow_Un1"
    target_folder = "10.Yellow_Un1"
    ...
    if query_exon == "Exon8": 
        ...
        last_exon = 1
        diff_number = 153                                                 #check if this is the last exon for searching stop codon 0 - not last exon 1 - last exon
...
#    print(done_species_list_temp)
    ...
    query_difference, start, stop, query_length, query_start_coor, query_stop_coor, complement, tblast_details = blast_and_check(gene,gene_folder, done_species_list, query_exon, frame, Species,scaffold, diff_number)
