...
scaffold = "OU342670.1"                                            #Scaffold where the gene is present, narrows the search space
Species = "Chrysoteuchia_culmella"                                      #Species to search                  
exon_numbers = "9"
scaff_area, start_area, end_area = 0,0, 5333774                  #if a small exon, and the area for the exon is known, use this to first search in the small area to make blast results easier first parameter - initialize the subsearch, start and end for the coordinates start and end
done_species_temp_switch, temp_species =0, ["Euproctis_similis"] 

exon_names = ["Exon1","Exon2","Exon3","Exon4","Exon5","Exon6","Exon7","Exon8","Exon9","Exon9"]
for i in range(len(exon_numbers)):
#    print
    query_exon = (exon_names[int(exon_numbers[i])-1])                                               #Exon to search    
  
    gene = "yellow_f4"                                                 #Gene to search    
    if query_exon == "Exon9": 
        ...
        last_exon = 1
        diff_number = 15                                                  #check if this is the last exon for searching stop codon 0 - not last exon 1 - last exon
    ...
    entries = os.listdir("C:/Users/sauba/Desktop/Work_Stuff/MRJP/Genes/3.Extracted/6.Yellow_f4")            #get a list of files present in exon output directory
...
#    print(done_species_list_temp)
    ...
    if query_difference < diff_number and query_difference != 0:
        ...
        if write_prompt.lower()[0] == "y":
            with open("C:/Users/sauba/Desktop/Work_Stuff/MRJP/Genes/3.Extracted/6.Yellow_f4/"+Species+"_"+query_exon+".fa",'w') as f:
